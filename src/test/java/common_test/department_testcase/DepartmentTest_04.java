 ...
 * 优化： 解决单个用例无法重复执行 解决办法一 ：利用时间戳动态设置部门名  解决办法二： 用例前置清除数据
 */
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class DepartmentTest_04 {
    ...
    void clearDepartment(){
        // 获取跟部门下的子部门列表
        ArrayList<Integer> idList = given()
                .queryParams("access_token",access_token,"id",1)
            ...
            idList.forEach(id -> {
                if(id != 1) {
                    Response response = given()
                            .queryParams("access_token", access_token, "id", id)
